...
void tap_sensing(SpiCore *spi_p, GpoCore *led_p)
{
    const uint8_t RD_CMD = 0x0b;
    const uint8_t DATA_REG = 0x08;
    ...
    int8_t xraw, yraw, zraw;
    float x, y, z;
    static float oldx_y_z[3] = {0, 0, 0};
    static float old_abs_sum = 0;
    ...
    if (oldx_y_z[0] != x || oldx_y_z[1] != y || oldx_y_z[2] != z)
    {    /*
        uart.disp("x/y/z axis g values: ");
        uart.disp(x, 3);
        uart.disp(" / ");
        ...
        //calculate the abs equation then compare readings to determine number of leds that will be on
        float abs_sum = 0;
        for(int i = 0; i < 3; i++)
        {
            if(oldx_y_z[i] < 0)
            {
                abs_sum += oldx_y_z[i] * -1.0;
            }
            else {
                abs_sum += oldx_y_z[i];
            }
        ...
        // minimum difference = 0.048, maximum difference = 0.661   no light tap below about 0.048, high tap is anything above the max
        float readings = abs_sum - old_abs_sum;
        if(readings < 0)
        {
            readings = readings * -1.0;
        }

        old_abs_sum = abs_sum;

        if(readings > 0.048 && readings <= 0.100) // light tap
        {
            led_p->write(1);//one led
        }
        else if(readings > 0.100 && readings <= 0.661) { // medium tap
            
            float magnitude = map( 0.1, 0.661, 1.0, 16.0, readings - 0.1);
            
            if(magnitude <= 2.0)
            {
                led_p->write(3); //3 <- 2 leds
            }
            else if(magnitude <= 3.0 && magnitude > 2.0)
            {
                led_p->write(7); //7 <- 3 leds
            }
            else if(magnitude <= 4.0 && magnitude > 3.0)
            {
                led_p->write(15); //15 <- 4 leds
            }
            else if(magnitude <= 5.0 && magnitude > 4.0)
            {
                led_p->write(31); //31 <- 5 leds
            }
            else if(magnitude <= 6.0 && magnitude > 5.0)
            {
                led_p->write(63); //63 <- 6 leds
            }
            else if(magnitude <= 7.0 && magnitude > 6.0)
            {
                led_p->write(127); //127 <- 7 leds
            }
            else if(magnitude <= 8.0 && magnitude > 7.0)
            {
                led_p->write(255); //255 <- 8 leds
            }
            else if(magnitude <= 9.0 && magnitude > 8.0)
            {
                led_p->write(511); //551 <- 9 leds
            }
            else if(magnitude <= 10.0 && magnitude > 9.0)
            {
                led_p->write(1023); //1023 <- 10 leds
            }
            else if(magnitude <= 11.0 && magnitude > 10.0)
            {
                led_p->write(2047); //2047 <- 11 leds
            }
            else if(magnitude <= 12.0 && magnitude > 11.0)
            {
                led_p->write(4095); //4095 <- 12 leds
            }
            else if(magnitude <= 13.0 && magnitude > 12.0)
            {
                led_p->write(8191); //8191 <- 13 leds
            }
            else if(magnitude <= 14.0 && magnitude > 13.0)
            {
                led_p->write(16383); //16,383 <- 14 leds
            }
            else if(magnitude <= 15.0 && magnitude > 14.0)
            {
                led_p->write(32767); //32,767 <- 15 leds
            }
            else if(magnitude <= 16.0 && magnitude > 15.0)
            {
                led_p->write(0xffff); //65,535 <- all lights up
            }
        }
        else if(readings > 0.661) // very strong tap
        {
            led_p->write(0xffff);//light up all the leds
        }
        else {
            led_p->write(0);
        }

    sleep_ms(2);//wait 2 milliseconds
}

//my map function, input (x1, x2, y1, y2, x), output y //note: this function assumes y1 is the intercept as well since those
