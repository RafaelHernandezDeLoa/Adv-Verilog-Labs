...
//different case senarios
//lab 6 chasing leds but with Pmod
void chasing_led(GpoCore *led_p, GpiCore *sw_p, double distance)
{
   static int i = 0; //for which direction led will go, 1 = right, 0 = left
   static int n = 0; //for which led we are turning on
   static double old_value = 0.0; //for sensing change in switch
   static double time = 0.0; //speed <- fix
...
//lab 9 the RGB LED display
void spec_disp(double distance, PwmCore *pwm_p)
{
    double reading = distance;

    pwm_p->set_freq(50);

    float cap = 0.5;
    //note: red is channel 2, green is channel 1, blue is channel 0
...
int main() {
   ...
   while (1) { /*
      ...
      display_double(&sseg, dist);
      //the extra functions
      /*
      chasing_led(&led, &sw, dist); */
      spec_disp(dist, &pwm);
   } //while
} //main
