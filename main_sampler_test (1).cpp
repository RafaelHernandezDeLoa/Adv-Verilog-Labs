...
//my lab code
void chasing_led(GpoCore *led_p, Ps2Core *ps2_p, SsegCore *sseg_p)
{
   static int i = 0; //for which direction led will go, 1 = right, 0 = left
   static int n = 0; //for which led we are turning on
   static int time = -1; //speed <- in this case, more time means slower blink rate
   static bool pause = false;

   int lbtn, rbtn, xmov, ymov;
   int j, old_xmov, new_time;

    //clear display
    for (j = 0; j < 8; j++)
    {
        sseg_p->write_1ptn(0xff, j);
    }
   ...
   if(time == -1)
   {
       ps2_p -> init();
       time = 138;
       sseg_p -> write_1ptn(sseg_p -> h2s(time % 10), 0); //first digit
       sseg_p -> write_1ptn(sseg_p -> h2s((int)(time / 10) % 10), 1); //second digit
       sseg_p -> write_1ptn(sseg_p -> h2s((int)(time / 100) % 10), 2); //third digit
       /*
    ...
    if(lbtn) //to toggle pause and resume
    {
        pause = !pause;
    }
    else if(rbtn)
    {
        ...
        while(rbtn) //wait till rbtn is released
        {
        ...
        while(!rbtn)
        {
            ...
            if(ps2_p->get_mouse_activity(&lbtn, &rbtn, &xmov, &ymov))
            {/*
                uart.disp("changing speed");
                uart.disp("\n\r");*/
                old_xmov += xmov;
                new_time = time - old_xmov;
                if (new_time < 0)
                    new_time = 0;
                else if (new_time > 999)
                    new_time = 999;

                sseg_p -> write_1ptn(sseg_p -> h2s(new_time % 10), 0); //first digit
                sseg_p -> write_1ptn(sseg_p -> h2s((int)(new_time / 10) % 10), 1); //second digit
                sseg_p -> write_1ptn(sseg_p -> h2s((int)(new_time / 100) % 10), 2); //second digit
            }
        }

        /*uart.disp("back to normal operations");
        uart.disp("\n\r");*/
    ...
    sseg_p -> write_1ptn(sseg_p -> h2s(time % 10), 0); //first digit
    sseg_p -> write_1ptn(sseg_p -> h2s((int)(time / 10) % 10), 1); //second digit
    sseg_p -> write_1ptn(sseg_p -> h2s((int)(time / 100) % 10), 2); //second digit

    if(pause)//stay in the right spot
    {
        n = 0;
